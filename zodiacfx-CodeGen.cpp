...
bool CodeGenInspector::preorder(const IR::Declaration_Variable* decl) {
    auto type = ZODIACFXTypeFactory::instance->create(decl->type);
...
bool CodeGenInspector::comparison(const IR::Operation_Relation* b) {
    ...
    auto et = ZODIACFXTypeFactory::instance->create(type);

    bool scalar = (et->is<ZODIACFXScalarType>() &&
                   ZODIACFXScalarType::generatesScalar(et->to<ZODIACFXScalarType>()->widthInBits()))
                  || et->is<ZODIACFXBoolType>();
    if (scalar) {
        builder->append("(");
        visit(b->left);
        builder->spc();
...
bool CodeGenInspector::preorder(const IR::Cast* c) {
    ...
    builder->append("(");
    builder->append("(");
    auto et = ZODIACFXTypeFactory::instance->create(c->destType);
    et->emit(builder);
    builder->append(")");
    visit(c->expr);
    builder->append(")");
...
bool CodeGenInspector::preorder(const IR::Type_Typedef* type) {
    auto et = ZODIACFXTypeFactory::instance->create(type->type);
...
bool CodeGenInspector::preorder(const IR::AssignmentStatement* a) {
    ...
    auto zodiacfxType = ZODIACFXTypeFactory::instance->create(ltype);
    bool memcpy = false;
...
void CodeGenInspector::widthCheck(const IR::Node* node) const {
    ...
    if (tb->size % 8 == 0 && ZODIACFXScalarType::generatesScalar(tb->size))
        return;
