...
class StateTranslationVisitor : public CodeGenInspector {
    ...
    const ZODIACFXParserState* state;

    void compileExtractField(const IR::Expression* expr, cstring name, unsigned alignment, ZODIACFXType* type);
    void compileExtract(const IR::Expression* destination);
    void compileLookahead(const IR::Expression* destination);

 public:
    explicit StateTranslationVisitor(const ZODIACFXParserState* state) :
...
StateTranslationVisitor::compileExtractField(
    const IR::Expression* expr, cstring field, unsigned alignment, ZODIACFXType* type) {
    ...
    if (widthToExtract != loadSize) {
        ...
        builder->emitIndent();
        visit(expr);
        builder->appendFormat(".%s &= ZODIACFX_MASK(", field.c_str());
        type->emit(builder);
...
StateTranslationVisitor::compileExtract(const IR::Expression* destination) {
    ...
    if (ht->is<IR::Type_Header>()) {
        builder->emitIndent();
        visit(destination);
        builder->appendLine(".zodiacfx_valid = 1;");
    }
}
...
void ZODIACFXParserState::emit(CodeBuilder* builder) {
    StateTranslationVisitor visitor(this);
    visitor.setBuilder(builder);
    state->apply(visitor);
}

ZODIACFXParser::ZODIACFXParser(const ZODIACFXProgram* program, const IR::ParserBlock* block, const P4::TypeMap* typeMap) :
        program(program), typeMap(typeMap), parserBlock(block),
        packet(nullptr), headers(nullptr), headerType(nullptr) {}

void ZODIACFXParser::emitDeclaration(CodeBuilder* builder, const IR::Declaration* decl) {
...
void ZODIACFXParser::emit(CodeBuilder* builder) {
...
bool ZODIACFXParser::build() {
    ...
    for (auto state : parserBlock->container->states) {
        auto ps = new ZODIACFXParserState(state, this);
        states.push_back(ps);
    }
    ...
    if (ht == nullptr)
        return false;
    headerType = ZODIACFXTypeFactory::instance->create(ht);
    return true;
}
